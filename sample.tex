...
The generally accept scheme in human activity recognition is to use some supervised machine learning techniques.
The interested activities vary from gestures and and simple activities such as walking, running in earlier researches \cite{kwapisz2011activity, liang1998real, mantyjarvi2001recognizing} to more complex daily living activities\cite{rai2012mining}.
...
Another line of research aims at discovering activity patterns from unlabeled data.
...
Other researches \cite{rashidi2011discovering, aztiria2012discovering} also infers ADLs from sequential information.
Huynh et al \cite{huynh2006unsupervised} and Minnen et al \cite{minnen2006discovering} detected certain simple activities from on-body acceleration data in an unsupervised way.

Topic model is a popular method in unsupervised activity discovery.
Farrahi et al. \cite{farrahi2009learning} inferred daily routines from location transition and proximity using topic models.
Another typical way to use topic model in activity discovery is a hierarchical approach.
...
A quantified investigation on topic model based activity discovery is given by Julia Seiter et al.\cite{seiter2014discovery}.

Similar approaches are also used in mining video stream \cite{wang2009human}.
...
\begin{table}[htbp]
    ...
    \multicolumn{1}{l}{Lying} &  \multicolumn{1}{l}{} \\
    \bottomrule
    \end{tabular}%
